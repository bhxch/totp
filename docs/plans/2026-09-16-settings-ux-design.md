...
- 执行前计算 vault JSON sha256 与基线比对：本地备份新增 `lastBackupHash` 基线（平台本地）；云同步复用现有 `cloudRev`/hash 机制（`syncOrchestrator.ts:52-92`）
- 相同 → 跳过写入，仅更新「上次检查时间」
...
- 每步 TDD、独立 commit；core 变更（变更检测、路径解析、settings 字段）配单测；UI 配既有组件测试模式；终局跑既有 E2E 与构建
