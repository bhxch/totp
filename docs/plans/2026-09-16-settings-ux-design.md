...
- **收敛规则**：最后被采纳（downloaded/conflict-resolved）的目标胜出，其余目标与胜者对齐推送——多目标最终收敛到同一份最新数据，防目标间互相打架（如两台设备各写了一个后端）
  - 勘误（2026-09-18 审查裁定）：原表述「本轮第一个产生新 vault 的目标作为基准」与实现（`multiTarget.ts` 顺序遍历，后采纳者覆盖为当前内容）不符；两者为任意平局裁决，冲突副本链保证败者内容不丢，按实现对齐文档
...
8. **§3.2「用新口令重置云端」原计划漏项，终审修复落地**：换备份口令后云端对象仍为旧口令信封，同步该目标报「口令不匹配」，原实现无救济入口。已在 CloudCard 以按目标行内重置操作补齐——失败状态显示「失败：口令不匹配」并提供「用当前口令重置云端」行内操作，经行内两步确认后以当前会话口令 `pushEnvelope` 重新加密覆盖云端对象、以新信封 hash 落基线（重置确认挂起期间同步按钮禁用防重入）。
9. **§8.1 挂账清理（2026-09-17，四批）**：第 6 条所列 M3 审查挂账六项已全部落地（commit 范围 c438d68..344674d）——BackupCard 原生 radio 接 MdSegmentedButton（F5）、interval 原生 select 以新组件 MdSelect 收口并接线双卡（F6）、MdSwitch 未选中拇指 16dp、rail/tabs 标签接 label-medium（第 5 条「label-medium 零引用预留」就此了结）、after 证据图入库、X9 弹层（MdDialog/MdMenu）抽查闭环；同批落地自动备份/同步「跳过」可观测三态（`recordStatus` ok: null，锁定/无口令/未启用目标显示跳过原因，内容无变化静默跳过）与 CloudCard 添加目标菜单化、目标行移除、凭据回存单目标合并、状态 summary 中文化。仍开放：BackupCard/CloudCard 之外的原生控件（ImportCard select/radio、EntryForm select）、MdMenu 方向键导航/aria-haspopup 等 a11y 欠账、mac/Linux 真机验证（见审查报告第八节）。
10. **§8.1 挂账清理批 5/6（2026-09-17）**：第 9 条「仍开放」再收口——ImportCard/EntryForm 原生控件清零（ImportCard 手动格式/scheme select 接 MdSelect、冲突策略三 radio 接 MdSegmentedButton；EntryForm 类型/算法/匹配策略 select 接 MdSelect，fieldset 语义分组裁定保留；da3238c），MdMenu a11y 补齐（方向键循环/Home-End/开启聚焦首项/role=menuitem/Tab 关闭/Esc 经 triggerEl 回焦，及 CloudCard/OtpListItem 消费端 aria-haspopup/expanded 与键盘可达右键入口；b409669，外点关闭前已由 5368bb0 补齐）。rg 复核全仓 .vue 无原生 select/radio，仍开放仅 mac/Linux 真机验证（见审查报告第八节）。
