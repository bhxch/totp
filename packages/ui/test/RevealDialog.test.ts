...
describe('RevealDialog', () => {
  it('open=false 不渲染弹层', () => {
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: false, entry: null } })
    expect(w.find('.md-dialog').exists()).toBe(false)
  })

  it('entry=null 时 headline 兜底为「密钥」（不渲染 undefined 字样）', () => {
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: true, entry: null } })
    expect(w.find('.md-dialog__headline').text()).toBe('密钥')
    expect(w.text()).not.toContain('undefined')
  })

  it('headline「{issuer} — 密钥」；密钥呈前 4…后 4 遮蔽形态，不渲染完整 secret', () => {
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: true, entry } })
  ...
  it('secret ≤8 位时全显', () => {
    ...
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: true, entry: short } })
    expect(w.find('.reveal-secret').text()).toBe('ABCD2345')
  })

  it('secret 含空白时归一化后遮蔽', () => {
    const spaced = { ...entry, secret: 'JBSW Y3DP EHPK 3PXP' }
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: true, entry: spaced } })
    expect(w.find('.reveal-secret').text()).toBe('JBSW…3PXP')
  })

  it('点「关闭」（data-md-close 委托）emit close', async () => {
    const w = mount(RevealDialog, { global: { plugins: [createTestI18n()] }, props: { open: true, entry }, attachTo: document.body })
