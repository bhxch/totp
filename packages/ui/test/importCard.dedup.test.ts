...
describe('ImportCard 去重接线', () => {
  it('完全重复文件重导入：全部 identical → 零落库，report 显示自动跳过数', async () => {
    ...
    const w = mount(ImportCard, { props: { platform: mkPlatform(store, vaultUri('JBSWY3DPEHPK3PXP', 'GitHub', 'me@x.com')) } })
    await gotoConfirm(w, '')
  ...
  it('同 secret 异 label：suspect 默认跳过，不落库', async () => {
    const store = await readyStore()
    const w = mount(ImportCard, { props: { platform: mkPlatform(store, vaultUri('JBSWY3DPEHPK3PXP', 'Other', 'other@y.z')) } })
    await gotoConfirm(w, '')
  ...
  it('同 issuer+label 异 secret：conflict 三选路径不回归（replace 覆盖目标条目）', async () => {
    const store = await readyStore()
    const originalUuid = store.vault.entries[0]!.uuid
    const w = mount(ImportCard, { props: { platform: mkPlatform(store, vaultUri('JBSWY3DPEHPK3PXQ', 'GitHub', 'me@x.com')) } })
    await gotoConfirm(w, '')
  ...
  it('文件内完全重复行合并：confirm 提示合并数，落库不重复', async () => {
    ...
    const w = mount(ImportCard, { props: { platform: mkPlatform(store, `${line}\n${line}`) } })
    await gotoConfirm(w, '')
  ...
  it('commit 前 vault 变化：按最新 vault 重算判定树（预览 new → commit 时 identical 跳过）', async () => {
    const store = await readyStore()
    const w = mount(ImportCard, { props: { platform: mkPlatform(store, vaultUri('JBSWY3DPEHPK3PXQ', 'Fresh', 'f@x.com')) } })
    await gotoConfirm(w, '')
