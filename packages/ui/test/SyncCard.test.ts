...
describe('SyncCard', () => {
  ...
  it('开关关闭（即使未加密）或 hasEncryption 未提供：不显示明文同步警示', () => {
    const off = mount(SyncCard, { global: { plugins: [createTestI18n()] }, props: { platform: mkPlatform(null, { syncEnabled: false, hasEncryption: ref(false) }) } })
    expect(off.find('.warn').exists()).toBe(false)
    const unknown = mount(SyncCard, { global: { plugins: [createTestI18n()] }, props: { platform: mkPlatform(null, { syncEnabled: true }) } })
    expect(unknown.find('.warn').exists()).toBe(false)
  })

  it('I55：始终显示「同步开关按设备独立」per-device 提示', () => {
    const w = mount(SyncCard, { global: { plugins: [createTestI18n()] }, props: { platform: mkPlatform() } })
  ...
  it('I57：SyncStatus.pct 提供时显示「已用 X% / 100KB」；未提供时不显示', async () => {
    ...
    const w1 = mount(SyncCard, { global: { plugins: [createTestI18n()] }, props: { platform: withPct } })
  ...
  // T4 authFailed 警示已自本卡归位 CloudCard（spec §5 ⑤，T11）：原两例随职责迁移删除，
  // CloudCard 侧呈现见 cloudCard.conflict.test.ts
})
