...
export async function openBackupEnvelope(env: unknown, password: string): Promise<string> {
  ...
  // 边界与 securityStore 共用 KDF_DECRYPT_CLAMP（下限=OWASP 最低推荐；上限=已发布档位最大展开值，
  // 与写侧同源防漂移），未认证字段超限在 deriveKek（口令验证）之前按结构非法拒绝（F9）；
  // 钳的是展开值——kdf.profile 仅记录写入意图，不参与校验（向后兼容新档位名）
  if (
    (kdf.m !== undefined && (typeof kdf.m !== 'number' || kdf.m < KDF_DECRYPT_CLAMP.minM || kdf.m > KDF_DECRYPT_CLAMP.maxM)) ||
    (kdf.t !== undefined && (typeof kdf.t !== 'number' || kdf.t < KDF_DECRYPT_CLAMP.minT || kdf.t > KDF_DECRYPT_CLAMP.maxT)) ||
    (kdf.p !== undefined && (typeof kdf.p !== 'number' || kdf.p < KDF_DECRYPT_CLAMP.minP || kdf.p > KDF_DECRYPT_CLAMP.maxP))
