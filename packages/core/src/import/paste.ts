import { sniffAegis, sniffFoxauthEncrypted, sniffFormat } from './sniff'
...
import { importBitwarden, importFoxauthPlaintext, importProton, importStratum, importTwoFas } from './jsonApps'
...
// aegis/foxauth 加密需口令页——四者引导走导入页，不在此强解。
...
// foxauth 同理走同步明文变体 importFoxauthPlaintext；加密备份（isEncrypted）先以
// sniffFoxauthEncrypted 拦截引导至导入页口令通道（与 aegis 加密同款交互）。
const DISPATCH: Partial<Record<ImportFormat, (text: string) => ImportResult>> = {
  ...
  foxauth: importFoxauthPlaintext,
}
...
export function parsePastedText(text: string): PasteParseResult {
  ...
  if (fmt === 'foxauth' && sniffFoxauthEncrypted(text)) {
    return { unsupported: '加密 FoxAuth 备份请走导入页（需输入口令）' }
  }
