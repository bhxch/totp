...
- **Windows 自动解锁（DPAPI，仅桌面版）**：
  ...
  - 插件端不提供此方式
- **移除守护**：Passkey 与 Windows 自动解锁可随时移除；口令为兜底方式不可移除
- **换设备**：绑定数据（密文，含凭据 id/盐/包裹后的 DEK）随浏览器同步整体走；云备份/云同步（envelope）不含。跨设备能否用取决于凭据本身——Passkey 凭据在本机（平台认证器）时，他机无法求值 PRF，需在新设备重新绑定；可漫游凭据（如 YubiKey）可直接解锁；Windows 自动解锁（DPAPI）在他机静默失败。新设备通用路径：先用口令解锁，再重新绑定其他解锁方式
