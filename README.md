...
- **锁定触发器**（「安全」页「锁定策略」区，偏好持久化）：
  ...
  - 「空闲 N 分钟后锁定」（默认 0=禁用）：插件端在 options 页存活期每 30 秒轮询 `chrome.idle`（阈值即配置分钟数）；桌面端由空闲执行器判定。Firefox 端 idle API 运行时不可用，启动时提示降级、自动锁定不启用
...
- **Firefox（MV3 下降级运行）**：空闲/锁屏自动锁定不可用（idle API 运行时缺失，options 页启动时提示降级）；剪贴板自动清空不可用（无 offscreen API，清空降级为仅前台不调度）；Passkey（PRF）解锁支持有限，探测不支持时入口自动隐藏
