 ...
 *  ext.storage.session 键 'dek'（base64）——浏览器退出即清、默认不落盘，
...
export function createDekSession(): { get(): Promise<string | null>; set(dek: Uint8Array): Promise<void>; clear(): Promise<void> } {
  ...
  return {
    async get() {
      try {
        if (!ext) return null
        const o = await ext.storage.session.get(DEK_KEY)
        const raw = o[DEK_KEY]
    ...
    async set(dek) {
      ...
      try {
        if (!ext) return
        await ext.storage.session.set({ [DEK_KEY]: bytesToBase64(dek) })
      } catch {
    ...
    async clear() {
      try {
        if (!ext) return
        await ext.storage.session.remove(DEK_KEY)
      } catch {
